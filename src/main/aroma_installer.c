...
void ai_rebuildtxt(int cx, int cy, int cw, int ch)
{
	...
	actext_rebuild(ai_buftxt, cx, cy, cw, ch, ((buffer != NULL) ? buffer : ""), 0, 1);
	free(buffer);
}
...
void ai_dump_logs()
{
	...
	byte res = aw_confirm(ai_win, "Save Install Log", msgtext, "@alert", NULL, NULL);
	...
	if (res)
	{
		...
		else
		{
			...
			aw_alert(ai_win, "Save Install Log Error", "Install Logs has been saved...", "@alert", NULL);
		}
	}
}
...
void aroma_init_install(CANVAS *bg, int cx, int cy, int cw, int ch, int px, int py, int pw, int ph)
{
	...
	if (!atheme_draw("img.progress", bg, px, ai_prog_oy, pw, ai_prog_oh))
	{
		...
		ag_roundgrad(
		    bg, px + 1, ai_prog_oy + 1, pw - 2, ai_prog_oh - 2, ag_calculatealpha(acfg()->controlbg, 0xffff, 180),
