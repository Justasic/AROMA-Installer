...
#define _INITBACK()                                                                                                                                            \
	...
	if (aparse_last_back_view == 0)                                                                                                                            \
	{                                                                                                                                                          \
		if (!aparse_is_back_request)                                                                                                                           \
		{                                                                                                                                                      \
			is_back_request = 5;                                                                                                                               \
		}                                                                                                                                                      \
	}                                                                                                                                                          \
	...
	if (is_back_request != 5)                                                                                                                                  \
		is_back_request += transition_style;
...
#define _FREEARGS()                                                                                                                                            \
	int freearg_i;                                                                                                                                             \
	for (freearg_i = 0; freearg_i < argc; ++freearg_i)                                                                                                         \
		free(args[freearg_i]);                                                                                                                                 \
	free(args);
...
Value *AROMA_INSTALL(const char *name, State *state, int argc, Expr *argv[])
{
	...
	int ret_status = aroma_start_install(&aui_win_bg, 0, chkY, chkW + (pad * 2), chkH + pad, pad, btnY, chkW, bntH, &cvf, imgY, chkFY, chkFH + pad);
...
Value *AROMA_ALERT(const char *name, State *state, int argc, Expr *argv[])
{
	...
	_INITARGS();
	//-- Show Alert
	aw_alert(NULL, args[0], args[1], (argc > 2) ? args[2] : "", (argc > 3) ? args[3] : NULL);
	//-- Release Arguments
	_FREEARGS();
	//-- Return
	return StringValue(strdup(""));
...
Value *AROMA_CONFIRM(const char *name, State *state, int argc, Expr *argv[])
{
	...
	_INITARGS();
	//-- Show Confirm
	byte res = aw_confirm(NULL, args[0], args[1], (argc > 2) ? args[2] : "", (argc > 3) ? args[3] : NULL, (argc > 4) ? args[4] : NULL);
	//-- Release Arguments
	_FREEARGS();
...
Value *AROMA_TEXTDIALOG(const char *name, State *state, int argc, Expr *argv[])
{
	...
	_INITARGS();
	//-- Show Text Dialog
	aw_textdialog(NULL, args[0], args[1], (argc > 2) ? args[2] : NULL);
	//-- Release Arguments
	_FREEARGS();
	//-- Return
	return StringValue(strdup(""));
