...
/*
 ...
 GB R0 GB
*/
static const byte dither_tresshold_r[64] = {1, 7, 3, 5, 0, 8, 2, 6, 7, 1, 5, 3, 8, 0, 6, 2, 3, 5, 0, 8, 2, 6, 1, 7, 5, 3, 8, 0, 6, 2, 7, 1,
                                            0, 8, 2, 6, 1, 7, 3, 5, 8, 0, 6, 2, 7, 1, 5, 3, 2, 6, 1, 7, 3, 5, 0, 8, 6, 2, 7, 1, 5, 3, 8, 0};
static const byte dither_tresshold_g[64] = {1, 3, 2, 2, 3, 1, 2, 2, 2, 2, 0, 4, 2, 2, 4, 0, 3, 1, 2, 2, 1, 3, 2, 2, 2, 2, 4, 0, 2, 2, 0, 4,
                                            1, 3, 2, 2, 3, 1, 2, 2, 2, 2, 0, 4, 2, 2, 4, 0, 3, 1, 2, 2, 1, 3, 2, 2, 2, 2, 4, 0, 2, 2, 0, 4};
static const byte dither_tresshold_b[64] = {5, 3, 8, 0, 6, 2, 7, 1, 3, 5, 0, 8, 2, 6, 1, 7, 8, 0, 6, 2, 7, 1, 5, 3, 0, 8, 2, 6, 1, 7, 3, 5,
	...
	if (ag_32)
	{
		...
		for (y = 0; y < ag_fbv.yres; y++)
		{
			...
			for (x = 0; x < ag_fbv.xres; x++)
			{
				...
				int dxy     = yd + (x * agclp);
				ag_bf32[xy] = ag_rgb32(
				    ag_fbuf32[dxy + (colorspace_positions[0] >> 3)], ag_fbuf32[dxy + (colorspace_positions[1] >> 3)],
...
{
	return ag_rgb((byte)min(ag_r(c) * intensity, 255), (byte)min(ag_g(c) * intensity, 255), (byte)min(ag_b(c) * intensity, 255));
}
...
color ag_calculatealpha(color dcl, color scl, byte l)
{
	...
	return (color)(
	    (((ag_r(dcl) * fa) + (ag_r(scl) * na)) >> 11 << 11) | (((ag_g(dcl) * fa) + (ag_g(scl) * na)) >> 10 << 5)
...
color aAlphaB(color scl, byte l)
{
	...
	word na = l + (l >> 7);
	/* Calculate Alpha per channel */
	return (color)(((ag_r(scl) * na) >> 11 << 11) | ((ag_g(scl) * na) >> 10 << 5) | ((ag_b(scl) * na) >> 11));
}
	...
	if (ag_fb > 0)
	{
		...
		if (fadesz > 0)
		{
			...
			for (i = 1; i <= fadesz; i++)
			{
				...
				ag_draw_strecth_ex(&tmpb[i - 1], &cbg, xc - wtarget / 2, yc - htarget / 2, wtarget, htarget, 0, 0, agw(), agh(), scale, 1);
			}
		...
		for (x = bs_x; x < bs_x + bs_w; x++)
		{
			if ((x + ag_busypos) % (bs_h * 2) < bs_h)
			{
				...
				for (y = bs_y; y < bs_y + bs_h; y++)
				{
					...
					*((dword *)(ag_fbuf32 + dxy)) = (alp << colorspace_positions[0]) | (alp << colorspace_positions[1]) | (alp << colorspace_positions[2]);
				}
			}
		}
	}
...
void ag32fbufcopy(dword *bfbz)
{
	...
	for (y = 0; y < ag_fbv.yres; y++)
	{
		int yp = y * ag_fbv.xres;
		int yd = (ag_fbf.line_length * y);
		aMemcpyColorPos_neon((dword *)(ag_fbuf32 + yd), (dword *)bfbz + yp, ag_fbv.xres, 0);
	}
	...
	for (y = 0; y < ag_fbv.yres; y++)
	{
		...
		for (x = 0; x < ag_fbv.xres; x++)
		{
			...
			*((dword *)(ag_fbuf32 + yd + (x * agclp)))
			    = (ag_r32(bfbz[xy]) << colorspace_positions[0]) | (ag_g32(bfbz[xy]) << colorspace_positions[1]) | (ag_b32(bfbz[xy]) << colorspace_positions[2]);
		}
	}
...
{
	return (dword)(((r & 0xff) << 16) | ((g & 0xff) << 8) | (b & 0xff) | ((a & 0xff) << 24));
}
...
static char ag_colorsets[28][14]
    = {"#winbg",        "#winbg_g",     "#winfg",     "#winfg_gray", "#dialogbg",   "#dialogbg_g", "#dialogfg", "#textbg",      "#textfg",  "#textfg_gray",
       "#controlbg",    "#controlbg_g", "#controlfg", "#selectbg",   "#selectbg_g", "#selectfg",   "#titlebg",  "#titlebg_g",   "#titlefg", "#dlgtitlebg",
       "#dlgtitlebg_g", "#dlgtitlefg",  "#scrollbar", "#navbg",      "#navbg_g",    "#border",     "#border_g", "#progressglow"};
...
byte ag_check_escape(int *soff, const char **ssource, char *buf, byte realescape, byte *o)
{
	...
	else if ((off == '<') && ((*s == 'u') || (*s == 'i') || (*s == 'b') || (*s == 'q') || (*s == '*') || (*s == '@') || (*s == '#') || (*s == '/')))
	{
		...
		if (foundlt)
		{
			...
			if ((strcmp(tb, "u") == 0) || (strcmp(tb, "/u") == 0) || (strcmp(tb, "i") == 0) || (strcmp(tb, "/i") == 0) || (strcmp(tb, "b") == 0)
			    || (strcmp(tb, "/b") == 0) || (strcmp(tb, "q") == 0) || (strcmp(tb, "/q") == 0) || (strcmp(tb, "*") == 0) || (strcmp(tb, "/*") == 0)
			    || (strcmp(tb, "/#") == 0) || (strcmp(tb, "/@") == 0) ||
			    //-- ALIGN
			    (strcmp(tb, "@left") == 0) || (strcmp(tb, "@right") == 0) || (strcmp(tb, "@center") == 0) || (strcmp(tb, "@fill") == 0)
...
int ag_txt_getline(const char *s, int maxwidth_ori, byte isbig, byte *ischangealign, int *indent, int *next_indent, byte *endofstring)
{
	...
	while ((c = utf8c(s, &s, &move)))
	{
		if ((move == 1) && (ag_check_escape(&c, &s, tb, 1, &o)))
		{
			if (w > 0)
			{
				if ((strcmp(tb, "/@") == 0) || (strcmp(tb, "@left") == 0) || (strcmp(tb, "@right") == 0) || (strcmp(tb, "@center") == 0)
				    || (strcmp(tb, "@fill") == 0))
				{
