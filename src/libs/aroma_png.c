...
byte apng9_draw(CANVAS *_b, PNGCANVAS *p, int dx, int dy, int dw, int dh, APNG9P v, byte with_pad)
{
	...
	int drh  = min(rh, minH);
	//-- Top Left
	apng_stretch(_b, p, dx, dy, dlw, dlh, 1, 1, lw, lh);
	//-- Top Right
	apng_stretch(_b, p, (dx + dw) - drw, dy, drw, dlh, rx, 1, rw, lh);
	//-- Bottom Left
	apng_stretch(_b, p, dx, (dy + dh) - drh, dlw, drh, 1, ry, lw, rh);
	//-- Bottom Right
	apng_stretch(_b, p, (dx + dw) - drw, (dy + dh) - drh, drw, drh, rx, ry, rw, rh);
	//-- Top
	apng_stretch(_b, p, dx + dlw, dy, dw - (dlw + drw), dlh, v->x, 1, v->w, lh);
	//-- left
	apng_stretch(_b, p, dx, dy + dlh, dlw, dh - (dlh + drh), 1, v->y, lw, v->h);
	//-- Bottom
	apng_stretch(_b, p, dx + dlw, (dy + dh) - drh, dw - (dlw + drw), drh, v->x, v->y + v->h, v->w, rh);
	//-- Right
	apng_stretch(_b, p, (dx + dw) - drw, dy + dlh, drw, dh - (dlh + drh), v->x + v->w, v->y, rw, v->h);
	//-- Center
	apng_stretch(_b, p, dx + dlw, dy + dlh, dw - (dlw + drw), dh - (dlh + drh), v->x, v->y, v->w, v->h);
	return 1;
}

byte apng_stretch(
    CANVAS *_b, PNGCANVAS *p, int dx, int dy, int wDst, int hDst,
	...
	for (y = 0; y < hDst; y++)
	{
		...
		for (x = 0; x < wDst; x++)
		{
			...
			if (dstp != NULL)
			{
				...
				if (id2 < p->s)
				{
					dr = ((p->r[id1] * hc1 + p->r[id2] * hc2) * wc1 + (p->r[id3] * hc1 + p->r[id4] * hc2) * wc2) >> 14;
					dg = ((p->g[id1] * hc1 + p->g[id2] * hc2) * wc1 + (p->g[id3] * hc1 + p->g[id4] * hc2) * wc2) >> 14;
					db = ((p->b[id1] * hc1 + p->b[id2] * hc2) * wc1 + (p->b[id3] * hc1 + p->b[id4] * hc2) * wc2) >> 14;

					if (p->c == 4)
					{
						da           = ((p->a[id1] * hc1 + p->a[id2] * hc2) * wc1 + (p->a[id3] * hc1 + p->a[id4] * hc2) * wc2) >> 14;
						color dcolor = dstp[0];
						byte  falpha = da;
					...
					ag_setpixel(_b, dx + x, dy + y, ag_dodither_rgb(x, y, dr, dg, db));
				}
			}
