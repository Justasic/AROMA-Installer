 ...
 *
 */
byte INDR_translate_keyboard(AINPUTP me, INDR_DEVICEP dev, AINPUT_EVENTP dest_ev, struct input_event* ev)
{
	/* DUMP RAW EVENTS */
	...
	if (ev->type == EV_KEY)
	{
		...
		switch (ev->value)
		{
			case 0: dest_ev->state = AINPUT_EV_STATE_UP; break;

			case 1: dest_ev->state = AINPUT_EV_STATE_DOWN; break;
		}
		...
		switch (ev->code)
		{
			...
			case KEY_CAPSLOCK:
			case KEY_VOLUMEDOWN:
			case KEY_RIGHT: return AINPUT_EV_RET_DOWN; break;
		}
