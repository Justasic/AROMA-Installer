...
void INDR_dumpdev(INDR_DEVICEP dev)
{
	...
	LOGS("  VKN : %d, CALIB : (%d,%d,%d,%d)\n", dev->vkn, dev->p.xi.minimum, dev->p.xi.maximum, dev->p.yi.minimum, dev->p.yi.maximum);
}
 ...
 *
 */
static char* INDR_strtok_r(char* str, const char* delim, char** save_str)
{
	if (!str)
	{
		if (!*save_str)
 ...
 *
 */
static byte INDR_nonzero(const bytep array_s, dword startIndex, dword endIndex)
{
...
byte INDR_getdevclass(int fd)
{
	...
	byte ret = 0;
	/* Check Keyboard */
	byte haveKeyboardKeys = INDR_nonzero(keyBitmask, 0, INDR_SIZEOF_BIT_ARRAY(BTN_MISC))
	    || INDR_nonzero(keyBitmask, INDR_SIZEOF_BIT_ARRAY(KEY_OK), INDR_SIZEOF_BIT_ARRAY(KEY_MAX + 1));
	/* Check Gamepad */
	byte haveGamepadButtons = INDR_nonzero(keyBitmask, INDR_SIZEOF_BIT_ARRAY(BTN_MISC), INDR_SIZEOF_BIT_ARRAY(BTN_MOUSE))
	...
	{
		/* Multitouch */
		if (INDR_TEST_BIT(BTN_TOUCH, keyBitmask) || !haveGamepadButtons)
		{
			ret |= INDR_DEVCLASS_TOUCH;
			ret |= INDR_DEVCLASS_MULTITOUCH;
		}
	}
	else if (INDR_TEST_BIT(BTN_TOUCH, keyBitmask) && INDR_TEST_BIT(ABS_X, absBitmask) && INDR_TEST_BIT(ABS_Y, absBitmask))
	{
		/* Single Touch */
		ret |= INDR_DEVCLASS_TOUCH;
	}
	...
	if ((dev->devclass & INDR_DEVCLASS_TOUCH))
	{
		...
		if (dev->vkn > 0)
		{
			...
			for (i = 0; i < dev->vkn; i++)
			{
				...
				LOGS("  VIRTUALKEY[%s,%i] (%i,%i,%i,%i,%i)]\n", dev->file, i, dev->vks[i].scan, dev->vks[i].x, dev->vks[i].y, dev->vks[i].w, dev->vks[i].h);
			}
		}
	}
 ...
 *
 */
byte INDR_translate(AINPUTP me, INDR_DEVICEP dev, AINPUT_EVENTP dest_ev, struct input_event* ev)
{
	if (dev->devclass & INDR_DEVCLASS_TOUCH)
	{
