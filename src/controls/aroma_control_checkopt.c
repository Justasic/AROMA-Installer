...
void acchkopt_redrawitem(ACONTROLP ctl, int index)
{
	...
	else
	{
		...
		if (!drawed)
		{
			if (p->type)
			{
				ag_roundgrad(c, chkbox_x, chkbox_y, chkbox_s, chkbox_s, acfg()->controlbg_g, acfg()->controlbg, chkbox_s);
				ag_roundgrad(
				    c, chkbox_x + halfdp, chkbox_y + halfdp, chkbox_s - halfdp2, chkbox_s - halfdp2, acfg()->textbg, acfg()->textbg, chkbox_s - halfdp);

				if (p->id == d->selectedIndexs[p->group])
				{
					ag_roundgrad(
					    c, chkbox_x + halfdp2, chkbox_y + halfdp2, chkbox_s - (halfdp2 * 2), chkbox_s - (halfdp2 * 2), acfg()->selectbg, acfg()->selectbg_g,
					    chkbox_s - halfdp2);
				}
			}
			else
			{
				ag_roundgrad(c, chkbox_x, chkbox_y, chkbox_s, chkbox_s, acfg()->controlbg_g, acfg()->controlbg, 0);
				ag_roundgrad(c, chkbox_x + halfdp, chkbox_y + halfdp, chkbox_s - halfdp2, chkbox_s - halfdp2, acfg()->textbg, acfg()->textbg, 0);

				if (p->checked)
				{
					ag_roundgrad(
					    c, chkbox_x + halfdp2, chkbox_y + halfdp2, chkbox_s - (halfdp2 * 2), chkbox_s - (halfdp2 * 2), acfg()->selectbg, acfg()->selectbg_g, 0);
				}
			}
		}
	}
