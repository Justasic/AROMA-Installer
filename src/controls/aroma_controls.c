...
static char theme_name[AROMA_THEME_CNT][27]
    = {"img.background",    "img.titlebar",       "img.navbar",         "img.dialog",      "img.dialog.titlebar",   "img.progress",
       "img.prograss.fill", "img.selection",      "img.selection.push", "img.button",      "img.button.focus",      "img.button.push",
       "img.checkbox",      "img.checkbox.focus", "img.checkbox.push",  "img.checkbox.on", "img.checkbox.on.focus", "img.checkbox.on.push",
       "img.radio",         "img.radio.focus",    "img.radio.push",     "img.radio.on",    "img.radio.on.focus",    "img.radio.on.push"};
	...
	if (acfg()->fadeframes > 0)
	{
		...
		else if (anitype == 5)
		{
			...
			for (i = 1; i <= fadesz; i++)
			{
				...
				ag_draw_strecth_ex(&tmpb[i - 1], &win->c, xc - wtarget / 2, yc - htarget / 2, wtarget, htarget, x, pos, w, h, scale, 1);
			}
		...
		else if (anitype == 6)
		{
			...
			for (i = 1; i <= fadesz; i++)
			{
				...
				ag_canvas(&tmpb[i - 1], w, h);
				ag_draw_strecth_ex(&tmpb[i - 1], &win->c, w - wtarget, yc - htarget / 2, wtarget, htarget, x, pos, w, h, scale, 0);
		...
		else if (anitype == 7)
		{
			...
			for (i = 1; i <= fadesz; i++)
			{
				...
				int  xtarget = (w * scale) >> 8;
				ag_canvas(&tmpb[i - 1], w, h);
				ag_draw_strecth_ex(&tmpb[i - 1], &cbg, w - wtarget, yc - htarget / 2, wtarget, htarget, x, pos, w, h, scale, 0);
	...
	if (maskc != NULL)
	{
		...
		if (fadesz > 0)
		{
			...
			for (i = 1; i <= fadesz; i++)
			{
				...
				ag_draw_strecth_ex(&tmpb[i - 1], &cbg, xc - wtarget / 2, yc - htarget / 2, wtarget, htarget, x, y, w, h, scale, 1);
			}
...
{
	aw_alert(parent, "No Need Calibration", "This version doesn't need any calibration data...", NULL, NULL);
	return 1;
}
...
void aw_about_dialog(AWINDOWP parent)
{
	...
	snprintf(
	    ...
	    "Device:\n  <b><#selectbg_g>%s</#></b>\n"
	    "Update:\n  <b><#selectbg_g>%s</#></b>",
	    AROMA_NAME, AROMA_VERSION, AROMA_COPY, AROMA_BUILD, AROMA_BUILD_CN, AROMA_BUILD_L, AROMA_BUILD_A, AROMA_BUILD_URL, acfg()->rom_name,
	    acfg()->rom_version, acfg()->rom_author, acfg()->rom_device, acfg()->rom_date);
	aw_alert(parent, AROMA_NAME " " AROMA_VERSION, unchkmsg, "@install", NULL);
}
