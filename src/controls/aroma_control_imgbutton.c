...
ACONTROLP imgbtn_reinit(AWINDOWP win, ACONTROLP ctl, int x, int y, int w, int h, PNGCANVAS *img, char *text, byte isflat, byte touchmsg)
{
	...
	if (!isflat)
	{
		if (!atheme_draw("img.button", &d->control, 0, 0, w, h))
		{
			...
			ag_roundgrad(
			    &d->control, 1, 1, w - 2, h - 2, ag_calculatealpha(acfg()->controlbg, acfg()->winbg, 180),
			    ag_calculatealpha(acfg()->controlbg_g, acfg()->winbg, 160), (agdp() * acfg()->btnroundsz) - 1);
	...
	else
	{
		...
		if (!atheme_draw("img.button", &d->control_pushed, 0, 0, w, h))
		{
			...
			ag_roundgrad(
			    &d->control_pushed, wadd + 1, wadd + 1, w - (2 + wdel), h - (2 + wdel), acfg()->controlbg, acfg()->controlbg_g,
			    (agdp() * acfg()->btnroundsz) - 1);
			ag_roundgrad_ex(
			    &d->control_pushed, wadd + 1, wadd + 1, w - (2 + wdel), (h - (1 + wdel)) / 2, LOWORD(hl1), HIWORD(hl1), (agdp() * acfg()->btnroundsz) - 1, 1, 1,
			    0, 0);
		}
	...
	if (!isflat)
	{
		if (!atheme_draw("img.button.focus", &d->control_focused, 0, 0, w, h))
		{
			...
			ag_roundgrad(
			    &d->control_focused, wadd + 1, wadd + 1, w - (wdel + 2), h - (wdel + 2), acfg()->selectbg, acfg()->selectbg_g,
			    (agdp() * acfg()->btnroundsz) - 1);
			ag_roundgrad_ex(
			    &d->control_focused, wadd + 1, wadd + 1, w - (wdel + 2), (h - (wdel + 2)) / 2, LOWORD(hl1), HIWORD(hl1), (agdp() * acfg()->btnroundsz) - 1, 1,
			    1, 0, 0);
		}
...
{
	return imgbtn_reinit(win, NULL, x, y, w, h, img, text, isflat, touchmsg);
}