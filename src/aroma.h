...
byte apng_drawfont(
    CANVAS *_b, PNGFONTS *p, byte fpos, // Draw PNG Font Into Canvas
    int xpos, int ypos, color cl, byte underline, byte bold);
byte apng_draw_ex(
    CANVAS *_b, PNGCANVAS *p, int xpos,              // Draw PNG Font Into Canvas
...
byte ag_rectopa(
    CANVAS *_b, int x, int y, int w, int h, // Draw Solid Rectangle with Opacity
    color cl, byte l);
byte ag_draw(CANVAS *dc, CANVAS *sc, int dx, int dy); // Draw Canvas to Canvas
byte ag_draw_ex(
    CANVAS *dc, CANVAS *sc, int dx, int dy, // Draw Canvas to Canvas + Extra Arguments
    int sx, int sy, int sw, int sh);
byte ag_roundgrad(
    CANVAS *_b, int x, int y, int w, int h, // Draw Rounded & Gradient Rectangle
    color cl1, color cl2, int roundsz);
byte ag_roundgrad_ex(
    CANVAS *_b, int x, int y, int w, int h,      // Draw Rounded & Gradient Rectangle
...
byte ag_drawchar(
    CANVAS *_b, int x, int y, int c, // Draw Character into Canvas
    color cl, byte isbig);
...
byte ag_text(
    CANVAS *_b, int maxwidth, int x, int y, // Draw String into Canvas
...
byte ag_text_ex(
    ...
    byte forcecolor);
int ag_txtheight(
    int         maxwidth, // Calculate String Height to be drawn
...
//
int aroma_start_install(CANVAS *bg, int cx, int cy, int cw, int ch, int px, int py, int pw, int ph, CANVAS *cvf, int imgY, int chkFY, int chkFH);
...
//
void ac_regbounce(ACONTROLP ctl, int *scrollY, int maxScrollY);
void ac_regfling(ACONTROLP ctl, AKINETIC *akin, int *scrollY, int maxScrollY);
void ac_regpushwait(ACONTROLP ctl, int *moveY, int *flagpointer, int flagvalue);
void ac_regscrollto(ACONTROLP ctl, int *scrollY, int maxScrollY, int requestY, int *requestHandler, int requestValue);
...
ACONTROLP actext(AWINDOWP win, int x, int y, int w, int h, char *text, byte isbig);
ACONTROLP acbutton(AWINDOWP win, int x, int y, int w, int h, char *text, byte isbig, byte touchmsg);
ACONTROLP imgbtn(AWINDOWP win, int x, int y, int w, int h, PNGCANVAS *img, char *text, byte isflat, byte touchmsg);
ACONTROLP imgbtn_reinit(AWINDOWP win, ACONTROLP ctl, int x, int y, int w, int h, PNGCANVAS *img, char *text, byte isflat, byte touchmsg);
ACONTROLP accheck(AWINDOWP win, int x, int y, int w, int h);
...
ACONTROLP acopt(AWINDOWP win, int x, int y, int w, int h);
...
ACONTROLP accb(AWINDOWP win, int x, int y, int w, int h, char *textv, byte checked);
byte      accb_ischecked(ACONTROLP ctl);
ACONTROLP acmenu(AWINDOWP win, int x, int y, int w, int h, byte touchmsg);
...
ACONTROLP acchkopt(AWINDOWP win, int x, int y, int w, int h);
